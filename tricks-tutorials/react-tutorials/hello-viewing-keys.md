...
We now see that we are using the `onAccountAvailable` event which you can read more about [here](../../events/account-events.md). Inside our event we query the `viewingKeyManager` with our address contract `sscrt.at`, after that if a key already exists we assign it to state `setViewingKey`.
